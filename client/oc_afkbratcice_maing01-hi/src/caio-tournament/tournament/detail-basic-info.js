    ...
    return (
      <>
        <Uu5Elements.Block
          ...
          info={data.operativeList?.length > 0 || data.refereeList?.length > 0 ? [{ label: "", children: <Info data={data} /> }] : undefined}
        >
          <Uu5Elements.InfoGroup itemList={infoList} />
          ...
          {data.refereeList?.length > 0 && isCreated && (
            <>
              ...
              <div className={Config.Css.css({ display: "flex", gap: 8 })}>
                {data.refereeList.map((item) => <OcAuth.IdentityItem key={item} identity={item} subtitle={<Lsi lsi={{ cs: "Rozhodčí" }} />} />)}
              </div>
            </>
          )}
        </Uu5Elements.Block>
