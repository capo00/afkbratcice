...
| Design ECC | **Ladí se samostatně** (od 2026-09-06). Články na něj nečekají; obsahové stránky ano. Až vznikne, migrace je rozpad `sectionList` na dokumenty `ecc_section` a přesun textu z `content/pages.js` do databáze — obojí beze změny tvaru obsahu. Komponenty do obsahu půjdou z **`uu5bricksg01`** (2026-09-09), proto balíček zůstává v závislostech i přes svoji velikost. |
...
| **Rozsah migrace** | **Jen zápasy** (2026-09-09) — rozlosování, výsledky, sestavy a co k nim patří. **Články, fotogalerie ani soubory ke stažení se nemigrují**: jejich obsah není v databázi, ale v souborech na disku v0, takže je to nejdelší a nejkřehčí kus migrace kvůli obsahu, který klub dnes vydává jinak. Viz [migration.md](./migration.md), sekce 2. |
| **Ukončené sezóny** | **Vyhodnotí se jednou a uzavřou** (2026-09-09): `season.state = "closed"` + uložená konečná tabulka `finalTable`. Historie je ~2 500 zápasů, které už nikdo neupraví, a dopočítávat tabulku z roku 2009 při každém zobrazení znamená počítat ji z dat, o kterých se ví, že jsou neúplná. Živá sezóna se počítá dál ze zápasů. Viz [migration.md](./migration.md), sekce 2.1. |
...
| Loga týmů | **Od 2026-09-11 patří klubu, ne týmu** (víc věkových kategorií stejného klubu sdílí jeden erb) — `club.logoUri` drží URI z `sys_binary` (kolekce `club`), `team.clubId` je FK a `team.logoUri` je jen denormalizace pro čtení bez joinu. Když chybí, klient sáhne po klubovém erbu. Viz [data-model.md](./data-model.md), sekce 2. |
...
| 20 | ~~`Uu5Bricks` a `uu5codekitg01-forms` nejsou v závislostech~~ | – | **Vyřešeno 2026-09-07.** `uu5bricksg01` a `uu5codekitg01-forms@3.4.1` jsou v `client/package.json`; do import mapy je `createViteConfig()` dá sám (uzávěr `uu*` závislostí). Časová osa ověřená na `/historie`. Zbývá jen velikost: `public/libs` narostlo na 126 MB, z toho 70 MB je `uu_uubmldraw_iconsg04` pod `uu5bricksg01`. **Bricks zůstává** — přijdou z něj komponenty do obsahu pro ECC (2026-09-09) — takže se řeší jen vyřazení té jedné závislosti, viz `todo.md` 5.2. |
